...
public class BplusTreeLong implements ITreeIndex {
	...
	int headersize = HEADERPREFIX.length + 1 + BufferFile.INTSTORAGE * 3
	...
	/*public ArrayList<String> readTree() throws Exception {
	...
	public int Compare(String left, String right) throws Exception {
		return left.compareTo(right); // only lexicographic compare allowed
	}
	...
	public ArrayList<String> toHtml(String v1, String v2) throws Exception {
		...
		if (this.root == null) {
			...
			result = this.root.AsHtml(sb,v1,v2);
		}
		//return sb.toString();
                return result;
	}
	...
	public static class BplusNode {
		...
		// structure</param>
		// / <param name="splitString">if not null then the smallest key in the
		...
		public ArrayList<String> AsHtml(java.io.CharArrayWriter sb, String v1, String v2) throws Exception {
			...
			if (this.isLeaf) {
				for (int i = 0; i < this.Size; i++) {
					...
					if (key != null) {
						...
						if(!v1.equals("") && v2.equals("")) { //equalty query
							if(key.startsWith(v1)){
								String temp1 = key.substring(0, v1.length());
								String temp2 = key.substring(v1.length(),key.length());
								boolean ok=true;
								...
								if(ok)
								sb.write("equalty query: " + temp1 + " == " + v1 + "\n");
                                                                result.add(key+" "+seek);
							}
						}
						if(!v1.equals("") && !v2.equals("")){
							if(key.compareTo(v1)>=0 && key.compareTo(v2)<=1){
								sb.write("range query: " + key + " == " + v1 + "\n");
                                                                result.add(key+" "+seek);
							}
						}
				...
				if (this.ChildBufferNumbers[0] != BplusTreeLong.NULLBUFFERNUMBER) {
					...
					sb.write("<tr><td></td><td>" + this.ChildBufferNumbers[0]
							+ "</td><td>\r\n");
					this.MaterializedChildNodes[0].AsHtml(sb,v1,v2);
					sb.write("</td></tr>\r\n");
				}
				for (int i = 0; i < this.Size; i++) {
					String key = this.ChildKeys[i];
					...
					try {
						...
						sb.write("<tr><td></td><td>"
								...
								+ "</td><td>\r\n");
						this.MaterializedChildNodes[i + 1].AsHtml(sb,v1,v2);
						sb.write("</td></tr>\r\n");
